"""
Optimized: Generate ALL timeframes from tick data (2021-2026)
...
def process_year_ticks(year):
    """Load tick data for a specific year"""
    print(f"  Loading {year} ticks...", end=' ')
    ...
    files = sorted(ticks_dir.glob('eurusd_ticks_*.csv'))
    if not files:
        print("❌ No files")
        return None
...
def main():
    print("=" * 60)
    print("🚀 OPTIMIZED TIMEFRAME GENERATOR")
    print("=" * 60)
    ...
    for tf_name, candles in all_candles.items():
        if candles:
            ...
            print(f"   ✓ {filename:20s} - {len(candles):,} candles ({size:.2f} MB)")
